...
function httpclient() {
  ...
  this.perform = function(rurl, method, cb, data, exheaders, tlscb) {
    ...
    if(!clients[key]) {
      ...
      if(!curl.port) {
        switch(curl.protocol) {
          case "https:":
			client = https;
			client_options.host = curl.hostname;
			client_options.port = 443;
        //    client = http.createClient(443, curl.hostname,true);
        ...
        client = http; //http.createClient(parseInt(curl.port), curl.hostname);
		client_options.host = curl.hostname;
		client_options.port = parseInt(curl.port);
      }
    ...
    if(method === "PUT") {
		clients[key].headers["Content-Length"] = data.length;
		clients[key].headers["Content-Type"] = "text/javascript; charset=UTF-8";
	}
	...
	clients[key].options.method = method;
	clients[key].options.path = target;
    var req = clients[key].http.request(clients[key].options, function (res) {
	...
	for (var i in clients[key].headers) {
		req.setHeader(i, clients[key].headers[i]);
	}
